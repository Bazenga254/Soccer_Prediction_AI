...
async def analyze_match_for_jackpot(match: dict, league_cache: dict) -> dict:
    ...
    # Step 4: Get injuries and coach data (AI-verified with shared cache)
    injuries_home, injuries_away = [], []
    coach_home, coach_away = None, None
    _jk_formation_home, _jk_formation_away = None, None

    try:
        injuries_home, injuries_away, coach_home, coach_away, _jk_formation_home, _jk_formation_away = await _fetch_supplementary(
...
async def _fetch_supplementary(home_id: int, away_id: int, league_id: int,
                               ...
                               competition: str = ""):
    """Fetch injuries and coach data for both teams via AI-verified data (with fallback)."""
    # Try the data verifier first (shared SQLite cache benefits all users)
    try:
        import data_verifier
        verified = await data_verifier.get_verified_match_data(
            team_a_id=home_id, team_a_name=home_name,
            team_b_id=away_id, team_b_name=away_name,
            competition=competition, league_id=league_id,
        )
        if verified:
            return data_verifier.extract_for_match_stats(verified)
    except Exception as e:
        print(f"[Jackpot] Data verifier fallback: {e}")
